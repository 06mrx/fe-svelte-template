<script>
    ...
    function fetchDataFromUrlParams() {
        setTimeout(async () => {
            ...
            try {
                let apiUrl = `${PUBLIC_API_URL}/api/permissions?page=${currentPage}&per_page=${currentItemsPerPage}`;
                ...
                if (response.ok) {
                    ...
                    console.log(permissions.current_page);
    ...
    async function handleConfirmDelete() {
        ...
        try {
            const deleteUrl = `${PUBLIC_API_URL}/api/permissions/${selected?.id}`;
    ...
    function create() {
        goto('/app/admin/permissions/create');
    }

    function edit(userId) {
        goto(`/app/admin/permissions/${userId}/edit`);
    }
</script>
...
<div class="mx-auto w-full p-3 md:p-5">
    ...
    >
        <h2 class="text-3xl font-bold text-gray-800">Daftar Izin</h2>
        <div
    ...
    {:else if permissions.data?.length === 0}
        ...
        <div class=" rounded-lg bg-white shadow-lg"> <!-- Tambahkan overflow-x-auto untuk responsivitas horizontal -->
            <Table>
                ...
                <tbody class="divide-y divide-gray-200 bg-white">
                    {#each permissions?.data as data, i (data.id)}
                        <tr>
                            <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-900"
                                >{(permissions?.current_page - 1) * permissions?.per_page + i + 1}.
                            </td>
                            <td class="whitespace-nowrap px-6 py-4 text-sm text-gray-900">{data.name || '-'}</td>
                            ...
                            <td class="whitespace-nowrap px-6 py-4 text-right text-sm font-medium">
                                <EditButton onclick={() => edit(data.id)} tooltip="Edit" />
                                <DeleteButton tooltip="Delete" onclick={() => openDeleteModal(data)}></DeleteButton>
                            </td>
                        </tr>
                    {/each}
                </tbody>
