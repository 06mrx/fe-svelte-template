					...
					{#each datas?.data as article, i (article.id)}
						<article class="group cursor-pointer" onclick={() => show(article.id)}>
							...
							<div class="flex items-center space-x-3 mb-3">
								<div class="w-8 h-8 rounded-full flex items-center justify-center text-white text-sm font-medium {getRandomAvatar(article.author || 'Admin')}">
									{getInitials(article.author || 'Admin')}
								</div>
